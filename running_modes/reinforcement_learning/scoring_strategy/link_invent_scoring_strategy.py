...
class LinkInventScoringStrategy(BaseScoringStrategy):
    ...
    def _apply_scoring_function(self, sampled_sequences: List[SampledSequencesDTO], step) -> FinalSummary:
        molecules = self._join_linker_and_warheads_my(sampled_sequences, keep_labels=True)
        smiles = []
    ...
    def _join_linker_and_warheads_my(self, sampled_sequences: List[SampledSequencesDTO], keep_labels=False):
        ...
        for sample in sampled_sequences:
            ...
            # ### Raw Code down
            linker = self._attachment_points.add_attachment_point_numbers(sample.output, canonicalize=False)
            # decoration = self._attachment_points.add_attachment_point_numbers(sample.input, canonicalize=False)
